...
/**
 * @brief Escribe en un archivo .csv los datos de la nueva LinkedList generada de la editorial Minotauro
...
int controller_saveFromMinotauro(char* path, LinkedList* pArrayListMinotauro)
{
	...
	if(pFile != NULL)
	{
		parser_textFromMinotauro(pFile, pArrayListMinotauro);
		returnAux = 0;
	}
	fclose(pFile);
...
int controller_list_by_MinotauroEditorial(char* path,LinkedList* pArrayListBook, LinkedList* pArrayListMinotauro, LinkedList* pArrayListEditorial)
{
	...
	if((pArrayListBook != NULL) && (pArrayListEditorial != NULL))
	{
		...
		if(controller_saveFromMinotauro(path, pArrayListMinotauro) == 0)
		{
			controller_ListBook(pArrayListMinotauro, pArrayListEditorial);
			returnAux = 0;
		}
	}
