...
/**
 * @brief Escribe en un archivo los datos de la LinkedList de entidad eBook recibida por parametro
 * @param pFile Puntero a archivo a escribir
 * @param pArrayList Lista de entidad eBook
 * @return Retorna un valor logico para validar si hubo error (-1) o si salio todo ok (0)
 */
int parser_textFromLinkedList(FILE* pFile, LinkedList* pArrayList)
{
	int tam;
	int returnAux = -1;
	int idAux;
	...
	if(pFile != NULL)
	{
		...
		for(int i = 0; i < tam; i++)
		{
			bookAux = (eBook*) ll_get(pArrayList, i);
